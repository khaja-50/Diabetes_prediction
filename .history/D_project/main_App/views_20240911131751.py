    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            x_train, x_test, y_train, y_test = train_test_split(
                x, y, test_size=0.2, stratify=y, random_state=2)  # 0.2 20% test data
            print(x.shape, x_train.shape, x_test.shape)
            svmclassifier = svm.SVC(kernel='linear')
            # x_train is data and y_train is lebal
            svmclassifier.fit(x_train, y_train)
            from sklearn import metrics
            x_test_preduction = svmclassifier.predict(x_test)
            SVMP = svma = test_accuracy_score = accuracy_score(
                x_test_preduction, y_test)
            ...
            RFP = rfca = test_accuracy_score = accuracy_score(
                x_test_preduction, y_test)
            ...
            RFP = dtreea = test_accuracy_score = accuracy_score(
                x_test_preduction, y_test)
            ...
            S.Outcome = pre3
            S.save()
            context = {'result': S, 'svm': pre, 'rfc': pre2,
                       'dtree': pre3, 'svma': svma, 'rfca': rfca, 'dtreea': dtreea}
...
def Records(request):
    ...
    context = {'divisions': pdivsions, 'form': form,
